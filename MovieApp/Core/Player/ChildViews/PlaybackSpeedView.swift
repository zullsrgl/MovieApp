                ...
                HStack(alignment: .bottom){
                    ForEach(playbackRates.indices, id: \.self) { i in
                        Text("\(playbackRates[i], specifier: "%.2f")x")
                            .font(.headline)
                            .foregroundStyle( i == rateIndex ? .white : .gray)
                            .frame(width: geometryReader.size.width / CGFloat(playbackRates.count),
                                   alignment: .center)
                    }
                }
            }
...
#Preview {
    PlaybackSpeedView(rateIndex: .constant(2), onSelect: { _ in })
}
